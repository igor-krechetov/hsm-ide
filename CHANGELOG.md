...
## [0.14.0] - 2026-09-04
### New
- [R1] Support multiple ordered actions per state onEntry and onExit slot and per transition
- [R2] Add, edit, remove and reorder individual actions from the properties panel, each as one undoable step
- [R3] Serialize multiple entry/exit actions as multiple `<script>` elements within a single `<onentry>`/`<onexit>`, and multiple transition actions as multiple `<script>` elements
- [R4] Deserialize every `<script>` within an entry/exit block and every transition `<script>` into ordered action lists
- [R5] Render multiple actions on the diagram (bulleted list on states; `<multiple actions>` summary on transitions)
### Fix
...
### Refactoring
- Centralize action-list operations in `ModelActionUtils` and shared `StateMachineEntity` mutators
