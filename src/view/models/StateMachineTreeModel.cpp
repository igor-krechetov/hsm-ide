...
void StateMachineTreeModel::setupModelData() {
    // Build tree from StateMachineModel with root visible
    if (mModel && mModel->root()) {
        mRootNode = new TreeNode();
        mRootNode->entity = mModel->root();
        mRootNode->nodeType = TreeNode::NodeType::Root;
        mRootNode->parent = nullptr;

        // Add children of root state
        addRegularState(mRootNode, mModel->root());
    }
}
...
QModelIndex StateMachineTreeModel::index(int row, int column, const QModelIndex& parent) const {
    ...
    if (nullptr == parentNode) {
        ...
        childNode = parentNode->children.at(row);
    }
...
void StateMachineTreeModel::addModelEntity(TreeNode* parentNode, const QSharedPointer<model::StateMachineEntity>& entity) {
    if (nullptr != parentNode && entity) {
        ...
        qCritical() << "StateMachineTreeModel::addModelEntity: parentNode is null or entity is null!";
    }
}
