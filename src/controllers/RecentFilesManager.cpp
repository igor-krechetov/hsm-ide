...
RecentFilesManager::RecentFilesManager(const QString& configPath,
                                       QMenu* menuRecentHsm,
                                       QMenu* menuRecentLogs,
    ...
    , mMaxEntries(10)
    , mMenuRecentHsm(menuRecentHsm)
    , mMenuRecentLogs(menuRecentLogs)
    , mCallbackOpenRecentHSM(callbackOpenRecentHSM)
    , mCallbackOpenRecentLog(callbackOpenRecentLog) {
    loadRecentItems();
    updateMenuRecentHsm();
...
void RecentFilesManager::addPathToList(const QString& newPath,
...
void RecentFilesManager::updateMenuItems(QMenu* menu,
    ...
    if (!items.isEmpty()) {
        ...
        for (const QString& curPath : items) {
            ...
            connect(entryAction, &QAction::triggered, [this, callbackItem, curPath]() { callbackItem(curPath); });
...
void RecentFilesManager::updateMenuRecentHsm() {
    updateMenuItems(mMenuRecentHsm, mRecentHsmFiles, mCallbackOpenRecentHSM, [this]() { onActionClearRecentHSM(); });
}

void RecentFilesManager::updateMenuRecentLogs() {
    updateMenuItems(mMenuRecentLogs, mRecentLogFiles, mCallbackOpenRecentLog, [this]() { onActionClearRecentLogs(); });
}
